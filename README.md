...
| **三种叙事骨架** | 按材料类型选：介绍一件事（四段论证）/ 报告变化（F-I-I-J）/ 复盘（业务闭环五步） |
| 四段论证 | 解决什么问题 → 提供什么 → 相对优势 → 当前进展 |
| F-I-I-J | 事实 → 为什么重要 → 对业务的影响 → 我的判断。**日报周报走这条** |
| 结论 + 最多 3 论据 | 超过三条会互相稀释；今天只有一件重要的事就写一条 |
...
十三个命令，最常用的四个是 `suggest-terms`（抽你还没登记的业务说法）、`add-term`（一条命令入库，不用手写 YAML）、`profile`（认识你多少）、`extract-cases`（从改稿抽案例）；其余有 `route`、`lint`（含业务感密度）、`knowledge-query`（检索已确认知识）、`learn`（记录反馈事件）、`ingest`（语料抽候选）、`sanitize`（一致性脱敏）、`blind-packet`（多模型匿名评测）、`technique-query`（检索写作技巧）。
...
### 三、它会主动认识你（这是它和一份写作文档的区别）

**底座一致，材料不雷同。** 方法论对所有人一样，但词包、案例、偏好是你独有的。

你不需要记得去配置任何东西——**积累是 AI 主动做的**：

```
你：帮我改下这段周报
AI：（改完）……

    顺便记了一条：「为区域提供决策支持」→「以前要开三个表格来回对，
    现在圈完直接导出」，原因是抽象价值换成了能想象的动作。

    另外这次材料里有三个说法我还不认识，按理解归了类，你看对不对：
      1. 履约健康度看板 → 产品正式名（会被保护，模型不许给它改名）
      2. 超时履约率     → 交付物 / 指标
      3. 晨会前         → 业务场景（这类最值钱，能把读者放进那个时刻）
    全对就说「都对」，不想收的说「第 N 个不用」。

你：都对，第 3 个改成"每日晨会前"
```

一句话回完，词包就长了三个词。**下次它就认得，不用你再解释一遍**；
你说过不收的，它记住，不会再问第二遍。

随时可以问它「你现在认识我多少」：

```bash
python3 tools/business_language.py profile
```

它会报出认识多少业务词、几条偏好、几条真实案例，**以及下一步该补什么**。

### 四、两套 HTML 汇报模板（`visual/`）
